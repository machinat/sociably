...
const getRecognitionDataId = (data: RecognitionData) =>
  `@machinat/dialogflow:V0:${hashObject(data)}`;
  ...
  private async _updateAgent(agentData: RecognitionData) {
    ...
    if (!agent) {
      await agentsClient.setAgent({
        agent: {
          parent: projectPath,
          displayName: this.agentName || 'machinat-agent',
          description:
            'This agent is generated by @machinat/dialogflow package',
  ...
  private async _updateVersion(
    ...
    if (this.manualMode) {
      const [newVersion] = await versionsClient.createVersion({
        parent: agentPath,
        version: {
          description: `@machinat/dialogflow snapshot at ${new Date().toISOString()}`,
        },
      });
  ...
  private async _updateEnvironment(version: DialogFlowVersion) {
    ...
    if (!environment) {
      await environmentsClient.createEnvironment({
        ...
        environment: {
          description:
            'This evnironment is generated by @machinat/dialogflow package',
