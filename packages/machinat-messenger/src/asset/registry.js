...
class MessengerAssetsRegistry {
  ...
  getMessageCreativeId(tag: string): Promise<void | string> {
    return this.getAssetId(MESSAGE_CREATIVE, tag);
  }

  setMessageCreativeId(tag: string, id: string) {
    return this.setAssetId(MESSAGE_CREATIVE, tag, id);
  }

  getAllMessageCreatives() {
    return this.getAllAssets(MESSAGE_CREATIVE);
  }

  removeMessageCreativeId(tag: string) {
    return this.removeAssetId(MESSAGE_CREATIVE, tag);
  }
  ...
  async renderMessageCreative(
    ...
    if (existed !== undefined) {
      throw new Error(`message creative [ ${tag} ] already exist`);
    }

    const response = await this.bot.renderMessageCreative(node);
    if (response === null) {
      throw new Error(`message ${formatNode(node)} render to empty`);
    }

    const { message_creative_id: id } = response.results[0].body;
    await this.setMessageCreativeId(tag, id);
    return id;
  }

  getCustomLabelId(tag: string): Promise<void | string> {
    return this.getAssetId(CUSTOM_LABEL, tag);
  }

  setCustomLabelId(tag: string, id: string) {
    return this.setAssetId(CUSTOM_LABEL, tag, id);
  }

  getAllCustomLabels() {
    return this.getAllAssets(CUSTOM_LABEL);
  }

  removeCustomLabelId(tag: string) {
    return this.removeAssetId(CUSTOM_LABEL, tag);
  }

  async createCustomLabel(tag: string, body: Object): Promise<string> {
    const existed = await this.getCustomLabelId(tag);
    if (existed !== undefined) {
      throw new Error(`custom label [ ${tag} ] already exist`);
    }
    ...
    await this.setAssetId(CUSTOM_LABEL, tag, id);
    return id;
  }
  ...
  async deleteCustomLabel(tag: string): Promise<string> {
    ...
    if (id === undefined) {
      throw new Error(`custom label [ ${tag} ] not exist`);
    }
