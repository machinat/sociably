...
describe('as a FIFO queue', () => {
  test('with less job acquired once', async () => {
  ...
  test('with more job acquired once', async () => {
    queue.executeJobs(jobs.slice(0, 3)).then(batch1Resolved);
    queue.executeJobs(jobs.slice(3, 6)).then(batch2Resolved);
    queue.executeJobs(jobs.slice(6, 9)).then(batch3Resolved);
  ...
  test('with less job acquired asynchronizedly', async () => {
  ...
  test('with more job acquired asynchronizedly', async () => {
    jest.useRealTimers();
  ...
  test('with failed jobs return by acquire() asynchronizedly', async () => {
    ...
    await expect(Promise.all(promises)).resolves.toEqual([
      ...
      [...failedJobResponses([2]), ...failedJobResponses([3])],
      [...successJobResponses([4]), ...successJobResponses([5])],
      [...successJobResponses([6]), ...successJobResponses([7])],
      [...failedJobResponses([8])],
    ]);
  ...
  test('with each job acquired separately', async () => {
    jest.useRealTimers();
    ...
    expect(batch2Resolved).toHaveBeenCalledWith({
      success: false,
      errors: [new Error('Fail3')],
      batch: [undefined, ...successJobResponses([4]), undefined],
    });
    expect(batch3Resolved).toHaveBeenCalledWith({
      success: false,
      errors: [new Error('Fail6')],
      batch: [
        ...failedJobResponses([6]),
        ...successJobResponses([7]),
        undefined,
