...
test.each([
  ...
  expect(fooState.getAllStartWithKey('key')).resolves.toEqual(
    new Map([
      ['key1', 'bar'],
      ['key2', 'baz'],
    ]),
  );
  expect(fooState.getAllStartWithKey('key1')).resolves.toEqual(
    new Map([['key1', 'bar']]),
  );
  expect(fooState.getAllStartWithKey('xxx')).resolves.toEqual(new Map());
