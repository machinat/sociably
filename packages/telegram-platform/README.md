...
```bash
npm install @sociably/core @sociably/http @sociably/telegram-platform
# or with yarn
yarn add @sociably/core @sociably/http @sociably/telegram-platform
```
...
import Telegram from '@sociably/telegram-platform';

const { TELEGRAM_BOT_NAME, TELEGRAM_BOT_TOKEN, TELEGRAM_SECRET_TOKEN } =
  process.env;
