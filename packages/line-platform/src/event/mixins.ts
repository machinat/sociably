...
type MediaContentProvider = {
  /**
   * Provider of the image file. line: The image was sent by a LINE user. The
   * binary image data can be retrieved from the content endpoint. external: The
   * image was sent using the LIFF liff.sendMessages() method. For more
   * information, see liff.sendMessages() in the LIFF API reference.
   */
  type: 'external' | 'line';
...
export interface ThingsScenarioExecution {
  ...
  /**
   * Timestamp for when execution of scenario action started (milliseconds, LINE
   * app time)
   */
  readonly startTime: number;
  /**
   * Timestamp for when execution of scenario was completed (milliseconds, LINE
   * app time)
   */
