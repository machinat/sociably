...
In the previous lesson, we use `RegExp` to recognize `add todo <name>` command.
...
```diff title="src/handlers/handleChat.tsx"
    // ...
...
Let's look into `useIntent` services to see what happen.
...
`recognizer.detectText(channel, text)` method returns result like this:
...
Next, we'll learn how to proceed a complete dialog.
