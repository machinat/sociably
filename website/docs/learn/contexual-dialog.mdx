...
`set` prop of `PROMPT` receive two args: 1) a circumstance object containing current
...
We can also use another script like a function call. The `CALL` keyword execute
a script, and go back to the current point after it return.
...
We can use the it when a user doesn't have any todo like this:
