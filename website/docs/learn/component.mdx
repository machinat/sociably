...
Now we know how to use both general and native UIs. While the UIs are getting
more complex, we might want to reuse them to keep the codes [DRY](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself).
...
A **component** is a normal function with **capitalized** name that returns the
...
JSX element. Our `TodoList` function will receive a `{ todos: [/* todos... */] }`
object while rendering reply.
...
The second argument of the component function is a rendering info object.
...
The `WithMenu` component can be used with additional messages in the element
children. Like this:
