...
const TodoList = ({ todos }: TodoListProps, { platform }) => {
  if (todos.length === 0) {
    return <i>You have no todo now.</i>;
  }

  const summary = <i>You have <b>{todos.length}</b> todos:</i>;
...
The `{todos.map(todo => <i>{todo.name}</i>)}` code above actually
...
At the end of the function, we can return textual content as the default message:
...
const WithMenu = ({ children, todoCount }: WithMenuProps, { platform }) => {
  const info = <i>You have <b>{todoCount}</b> todos now.</i>;
...
The `<i>Hello! I'm a Todo Bot 🤖</i>` node is then available as `children`
