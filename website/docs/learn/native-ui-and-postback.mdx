...
One more thing to do is wiring the `handlePostback` handler with main event
...
event from the chat platform will flow through it. You can `.subscribe()` the
stream to listen events, or use `.pipe()` to create a new stream.
