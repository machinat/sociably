    ...
    async (
      ...
      return reply(
        <WithMenu todoCount={3}>
          {/* highlight-next-line */}
          <i>Hello{profile ? \`, \${profile.name}\` : ''}! I'm a Todo Bot 🤖</i>
        </WithMenu>
      );
    }
);
...
(
    getIntent,
    getUserProfile, // factory fn, receivces service instances
  ) =>
  async (context) => {
    /* ... */
  }; // handler fn, receivces event context
```
...
`useUserProfile` uses two built-in services: `BaseProfiler` and `StateRepository`.
...
`StateRepository` can access the user/chat/global state data from the storage.
Like:
...
It also requires `StateRepository` to save/load todos data.
...
async getTodos(
  ...
  const data = await this.stateRepository
    .threadState(thread)
      ...
      if (intent.type === 'finish') {
        ...
        return reply(
          <WithMenu todoCount={data.todos.length}>
            {todo ? (
              <i>Todo "<b>{todo.name}</b>" is done!</i>
            ) : (
              <i>This todo is closed.</i>
            )}
          </WithMenu>
        );
      }
      ...
      if (event.type === 'text') {
        ...
        if (matchingAddTodo) {
          ...
          return reply(
            // highlight-next-line
            <WithMenu todoCount={data.todos.length}>
              <i>Todo "<b>{todoName}</b>" is added!</i>
            </WithMenu>
          );
        }
      }\n
