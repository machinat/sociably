...
Machinat now supports three chat platforms: `messenger`, `telegram` and `line`.
...
The command initiates codes of a hello-world bot. Try `cd my-todo-bot` and take
a look.
...
The command registers some necessary settings to the Messenger platform. Now try
