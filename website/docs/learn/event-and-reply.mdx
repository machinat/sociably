...
change the element tag to `i`, `s` or `code` and see what's different. These
tags are **textual** and work like plain text so they can be placed in
`<p></p>` element.

For example `<b>Hello!</b> <s>I'm a</s> <i>Todo</i> <code>Bot 🤖</code>` message will look
like this:
...
For example this meassge can be replied no any chat platform:
...
You can check the full list of general tags [here](/docs/rendering-elements#general-tags).
