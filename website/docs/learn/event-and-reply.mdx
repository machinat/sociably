...
```tsx
  return reply(<i>Hello! I'm a <b>Todo Bot</b> 🤖</i>);
```

The html alike syntax is a **JSX** expression.
It represents a section of chat UI in a conversation.
Here the plain text becomes the message content, and `<b>...</b>` formats part
of that text.
...
For example, `<i><b>Hello!</b> <s>I'm a</s> Todo <code>Bot 🤖</code></i>` might look like this:
    ...
    async (
      ...
      if (event.type === 'text') {
        ...
        if (matchingAddTodo) {
          ...
          return reply(<i>Todo "<b>{todoName}</b>" is added!</i>);
        }
      }
