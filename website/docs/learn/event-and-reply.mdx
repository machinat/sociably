...
:::info
To use the JSX syntax in typescript, you must use `.tsx` for file extension.
And import `Machinat` at the beginning of the file like this:
...
event$
  .pipe(
    filter((ctx) => {
      // return `true` and allow only 'postback' event to pass
...
Our bot can receive events from user and reply simple answer now. Next, we'll learn
