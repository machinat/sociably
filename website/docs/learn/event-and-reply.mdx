...
Try interact with the bot, and it should now reply the string at `reply("...")`.
...
It represent a section of UI/UX in a conversation.
...
Also you need to import `Machinat` at the beginning of the file like:
...
:::info
**Messenger** and **LINE** platform don't support text format, it shows plain text on them.
:::

In the code above, `<b>...</b>` element make the inner text bold if chat platform supports it.
...
When an user sends a message to your bot, `handleChat` handler receives an event context object.
We can get the event infomations at `context.event`.
...
So we can extract the todo name with regular expression safely.
...
For example, change it to `{todoName.toUpperCase()}` and see what happen.
...
When replying a series of messages, you might want to shortly break between them.
We can use `Machinat.Pause` tag to do that.
...
Our bot can receive events in the chatroom and reply simple messages.
