...
We have learned how to ship features in a conversation. But sometimes chat UI
...
The web front-end codes are located at `<root>/webview` folder. Check
`webview/pages/index.tsx` file and you would see the webview client constructed
like this:
...
const client = new WebviewClient(
  typeof window === 'undefined'
    ? { mockupMode: true, authenticators: [] }
    : {
        authenticators: [new TelegramClientAuthenticator()],
      }
);
// ...
```
...
```tsx {2-19,35,40,47} title="webview/pages/index.tsx"
// ...
const TodoRow = ({ todo }) => (
    <tr>
...
```tsx {9-17} title="webview/pages/index.tsx"
const WebAppHome = () => {
  const data = useEventReducer(
    client,
    (currentData: null | TodoState, { event }): null | TodoState => {
...
combine **chat UI**, **services**, **dialog scripts** and **webview** to build
...
- Check our [note example](https://github.com/machinat/note-example/) to see how
  to serve in group chats with a [more complete webview project](https://github.com/machinat/note-example/blob/master/webview).
