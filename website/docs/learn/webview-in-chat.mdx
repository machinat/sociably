...
Here we add an `UrlButton` that links to the `/webview` endpoint of our server.
...
Here we add an `UrlButton` that links to the `/auth/telegram` endpoint of our server.
Note that the `login` prop has to be `true`.
...
After the `client` is constructed, it logs in the user and opens a connection to the server.
