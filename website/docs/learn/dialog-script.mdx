...
export default build(
  ...
  <$.BLOCK>
    {() => <i>Please enter new todo name:</i>}
  </$.BLOCK>
);
```
...
In the script body, the function `{() => <i>...</i>}` is a content node.
...
export default build<AskingFirstTodoVars>(
  ...
  <>
    {() => <i>You have no todo now. Do you want to create one?</i>}
